...
fn store_data(storable: StorableData) -> Result<Uuid, String> {
    ...
    storage::dump_json_string(DATA_PATH.to_string(), id, json).ok();
    Ok(id)
}
...
fn store_key(storable: StorableKey) -> Result<Uuid, String> {
    let id = Uuid::new_v4();
    let json = json::encode(&storable).unwrap();
    storage::dump_json_string(KEYS_PATH.to_string(), id, json).ok();
    Ok(id)
}
...
fn store_map(external_id: Uuid, storable: StorableMap) -> Result<(), String> {
    let id = external_id;
    let json = json::encode(&storable).unwrap();
    storage::dump_json_string(MAPS_PATH.to_string(), id, json).ok();
    Ok(())
}

pub fn load(external_id: &String) -> Vec<u8> {
    let map: StorableMap = json::decode(&storage::load_json_string(&MAPS_PATH.to_string(), external_id).unwrap()).unwrap();

    let (key_tx, key_rx) = channel();
    let key_id = map.key_id.to_string();
    thread::spawn(move || {
        key_tx.send(json::decode(&storage::load_json_string(&KEYS_PATH.to_string(), &key_id).unwrap()).unwrap()).unwrap();
    });

    let (data_tx, data_rx) = channel();
    let data_id = map.data_id.to_string();
    thread::spawn(move || {
        data_tx.send(json::decode(&storage::load_json_string(&DATA_PATH.to_string(), &data_id).unwrap()).unwrap()).unwrap();
    });
